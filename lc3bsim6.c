...
void MEM_stage()
{
	...
	if (Get_DATA_SIZE(PS.MEM_CS) == 0)
	{
		if (mask1(PS.MEM_ADDRESS) == 0)
		{
			LogicOut = PS.MEM_ALU_RESULT;
		}
		else
		{
			LogicOut = ((PS.MEM_ALU_RESULT & 0x00FF) << 8) + (PS.MEM_ALU_RESULT & 0x00FF);
		}
	}
	else
	{
	...
	if (V_DCACHE_EN)
	{
		dcache_access(Low16bits(PS.MEM_ADDRESS), &ReadWord, LogicOut, &DCacheR, WE0, WE1);
	}
	...
	if (Get_DATA_SIZE(PS.MEM_CS) == 0)
	{
		if (mask1(PS.MEM_ADDRESS) == 0)
		{
			LogicOutData = SEXT(8, ReadWord & (0x00FF));
		}
		else
		{
			LogicOutData = SEXT(8, (ReadWord >> 8) & (0x00FF));
		}
	}
	else
	{
		LogicOutData = ReadWord;
	}


	MEM_STALL = V_DCACHE_EN & (~DCacheR);
	TARGET_PC = PS.MEM_ADDRESS;
	TRAP_PC = LogicOutData;

	if (V_MEM_BR_STALL == 1) V_MEM_BR_STALL = 0;
	if (V_MEM_LD_REG == 1) V_MEM_LD_REG = 0;
	if (V_MEM_LD_CC == 1) V_MEM_LD_CC = 0;

	if (PS.MEM_V)
	{
		if (Get_MEM_LD_CC(PS.MEM_CS)) V_MEM_LD_CC = 1;
		else V_MEM_LD_CC = 0;

		if (Get_MEM_LD_REG(PS.MEM_CS)) V_MEM_LD_REG = 1;
		else V_MEM_LD_REG = 0;

		if (Get_MEM_BR_STALL(PS.MEM_CS)) V_MEM_BR_STALL = 1;
		else V_MEM_BR_STALL = 0;
	}
	...
	if (PS.MEM_V)
	{
		...
		{
			temp |= (PS.MEM_IR >> 9) & 0x07;
			temp &= PS.MEM_CC;
			if (temp != 0) MEM_PCMUX = 1;
		}
		else if (Get_UNCOND_OP(PS.MEM_CS) == 1) MEM_PCMUX = 1;
		else if (Get_TRAP_OP(PS.MEM_CS) == 1) MEM_PCMUX = 2;
		else MEM_PCMUX = 0;
	}
	else MEM_PCMUX = 0;
...
void AGEX_stage()
{
	...
	int ADDR1MuxOut = 0;
	int ADDR1MuxSelect = Get_ADDR1MUX(PS.AGEX_CS);
	if (ADDR1MuxSelect == 0) ADDR1MuxOut = PS.AGEX_NPC;
	else if (ADDR1MuxSelect == 1) ADDR1MuxOut = PS.AGEX_SR1;

	int ADDR2MuxOut = 0;
	int ADDR2MuxSelect = Get_ADDR2MUX(PS.AGEX_CS);
	if (ADDR2MuxSelect == 0) ADDR2MuxOut = 0;
	else if (ADDR2MuxSelect == 1) ADDR2MuxOut = Low16bits(SEXT(6, mask6(PS.AGEX_IR)));
	else if (ADDR2MuxSelect == 2) ADDR2MuxOut = Low16bits(SEXT(9, mask9(PS.AGEX_IR)));
	else if (ADDR2MuxSelect == 3) ADDR2MuxOut = Low16bits(SEXT(11, mask11(PS.AGEX_IR)));

	int PlusOut = 0;
	int LSHFSelect = Get_LSHF1(PS.AGEX_CS);
	if (LSHFSelect == 0) PlusOut = Low16bits(ADDR1MuxOut + ADDR2MuxOut);
	else if (LSHFSelect == 1) PlusOut = Low16bits(ADDR1MuxOut + (ADDR2MuxOut << 1));

	int ADDRESSMUXOut = 0;
	int ADDRESSMUXSelect = Get_ADDRESSMUX(PS.AGEX_CS);
	if (ADDRESSMUXSelect == 0) ADDRESSMUXOut = Low16bits((PS.AGEX_IR & 0xFF) << 1);
	else if (ADDRESSMUXSelect == 1) ADDRESSMUXOut = Low16bits(PlusOut);

	int SR2MUXOut = 0;
	int SR2MUXSelect = Get_SR2MUX(PS.AGEX_CS);
	if (SR2MUXSelect == 0) SR2MUXOut = PS.AGEX_SR2;
	else if (SR2MUXSelect == 1) SR2MUXOut = Low16bits(SEXT(5, mask5(PS.AGEX_IR)));

	int SHFOut = 0;
	if (mask2(PS.AGEX_IR >> 4) == 0) SHFOut = Low16bits(PS.AGEX_SR1 << mask4(PS.AGEX_IR));
	else if (mask2(PS.AGEX_IR >> 4) == 1) SHFOut = Low16bits(PS.AGEX_SR1 >> mask4(PS.AGEX_IR));
	else if (mask2(PS.AGEX_IR >> 4) == 2) SHFOut = Low16bits(PS.AGEX_SR1 << mask4(PS.AGEX_IR));
	else if (mask2(PS.AGEX_IR >> 4) == 3)
	{
		SHFOut = PS.AGEX_SR1;
		int amountToShift = mask4(PS.AGEX_IR);
		int signBit = (PS.AGEX_SR1 & 0x8000);
	...
	int ALUOut = 0;
	int ALUSelect = Get_ALUK(PS.AGEX_CS);
	if (ALUSelect == 0) ALUOut = Low16bits(SR2MUXOut + PS.AGEX_SR1);
	else if (ALUSelect == 1) ALUOut = SR2MUXOut & PS.AGEX_SR1;
	else if (ALUSelect == 2) ALUOut = SR2MUXOut ^ PS.AGEX_SR1;
	else if (ALUSelect == 3) ALUOut = SR2MUXOut;

	int ResultMuxOut = 0;
	int ResultMuxSelect = Get_ALU_RESULTMUX(PS.AGEX_CS);
	if (ResultMuxSelect == 0) ResultMuxOut = SHFOut;
	else if (ResultMuxSelect == 1) ResultMuxOut = ALUOut;

	if (V_AGEX_BR_STALL == 1) V_AGEX_BR_STALL = 0;
	if (V_AGEX_LD_REG == 1) V_AGEX_LD_REG = 0;
	if (V_AGEX_LD_CC == 1) V_AGEX_LD_CC = 0;

	if (PS.AGEX_V)
	{
		if (Get_AGEX_LD_CC(PS.AGEX_CS)) V_AGEX_LD_CC = 1;
		else V_AGEX_LD_CC = 0;

		if (Get_AGEX_LD_REG(PS.AGEX_CS)) V_AGEX_LD_REG = 1;
		else V_AGEX_LD_REG = 0;

		if (Get_AGEX_BR_STALL(PS.AGEX_CS)) V_AGEX_BR_STALL = 1;
		else V_AGEX_BR_STALL = 0;
	}


	int LD_MEM = (~MEM_STALL) & 0x01;
	...
	if (LD_MEM)
	{
		...
		NEW_PS.MEM_ALU_RESULT = ResultMuxOut;
...
void DE_stage()
{
	...
	int* row = CONTROL_STORE[CONTROL_STORE_ADDRESS];

	int SR1N = Get_SR1_NEEDED(row);
	int SR2N = Get_SR2_NEEDED(row);
	int DRM = Get_DRMUX(row);
	int BRS = Get_DE_BR_STALL(row);

	int ii, jj = 0;

	if ((BRS == 1) && (PS.DE_V == 1)) V_DE_BR_STALL = 1;
	else V_DE_BR_STALL = 0;

	int SR2IDOut = 0;
	if (((PS.DE_IR >> 13) & 0x01) == 1)
	{
		SR2IDOut = ((PS.DE_IR >> 9) & 0x07);
	}
	else SR2IDOut = (PS.DE_IR & 0x07);

	int IR_86 = (PS.DE_IR >> 6) & 0x07;

	if (DEP_STALL == 1) DEP_STALL = 0;
	if (PS.DE_V == 1)
	{
		if ((V_AGEX_LD_CC || V_MEM_LD_CC || V_SR_LD_CC) && Get_DE_BR_OP(row)) DEP_STALL = 1;
		else if ((SR1N == 1) && (V_AGEX_LD_REG || V_MEM_LD_REG || V_SR_LD_REG))
		{
			if (IR_86 == PS.AGEX_DRID || IR_86 == PS.MEM_DRID || IR_86 == PS.SR_DRID)
			{
				DEP_STALL = 1;
			}
			else DEP_STALL = 0;
		}
		else if ((SR2N == 1) && (V_AGEX_LD_REG || V_MEM_LD_REG || V_SR_LD_REG))
		{
			if (SR2IDOut == PS.AGEX_DRID || SR2IDOut == PS.MEM_DRID || SR2IDOut == PS.SR_DRID)
			{
				DEP_STALL = 1;
			}
			else DEP_STALL = 0;
	...
	int SR1Out = REGS[IR_86];
	int SR2Out = REGS[SR2IDOut];
	int AGEXV = 0;
	...
	int DRMuxOut = 0;
	if (DRM == 0) DRMuxOut = (PS.DE_IR >> 9) & 0x07;
	else DRMuxOut = 0x07;

	int LD_AGEX = (~MEM_STALL) & 0x01;
 	if (PS.DE_V == 1 && DEP_STALL == 0) AGEXV = 1;

	if (LD_AGEX)
	{
		NEW_PS.AGEX_NPC = PS.DE_NPC;
		NEW_PS.AGEX_IR = PS.DE_IR;
		NEW_PS.AGEX_SR1 = SR1Out;
		NEW_PS.AGEX_SR2 = SR2Out;
		NEW_PS.AGEX_CC = (N << 2) + (Z << 1) + (P);
		NEW_PS.AGEX_DRID = DRMuxOut;
		NEW_PS.AGEX_V = AGEXV;
...
void FETCH_stage()
{
      int PCMUXOUT;                                         //Output of PCMUX.
      int LD_PC;                                            //PC load signal.
      int LD_DE;                                            //DE load signal.
      int DV;                                               //Decode stage valid bit.

      int readWord;                                         //Variable for word from cache.
      int logicOut;                                         //Input for DE_V.

      icache_access(PC, &readWord, &ICACHE_R);               //Access cache.

      int bubble = 0;                                       //Bubble determination.
	bubble |= ((~ICACHE_R) & 0x0001);
	bubble |= V_DE_BR_STALL;
	bubble |= V_AGEX_BR_STALL;
	bubble |= V_MEM_BR_STALL;		                  //Bubble inserted if cache miss.

      int stall = 0;                                        //Stall determination.

      stall = DEP_STALL;                                    //Stall.
      stall |= MEM_STALL;

      if (stall)                                            //If stall, stop propagation.
      {
            LD_PC = 0;
            LD_DE = 0;
            logicOut = 0;
      }
      else if (bubble)                                      //If bubble, invalidate and propagate.
      {
            LD_PC = 0;
            LD_DE = 1;
            logicOut = 0;
	...
	if (V_MEM_BR_STALL == 1 && MEM_PCMUX != 0) LD_PC = 1;

      if (MEM_PCMUX == 0) PCMUXOUT = PC + 2;                //PCMUX emulation.
      else if (MEM_PCMUX == 1) PCMUXOUT = TARGET_PC;
      else if (MEM_PCMUX == 2) PCMUXOUT = TRAP_PC;

      if (LD_DE)                                            //Update latches if needed.
      {
            NEW_PS.DE_NPC = PC + 2;
            NEW_PS.DE_IR = readWord;
            NEW_PS.DE_V = logicOut;
      }

      if (LD_PC) PC = PCMUXOUT;                             //Update PC if needed.
}
